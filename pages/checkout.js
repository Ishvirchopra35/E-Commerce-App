...
export default function CheckoutPage() {
    ...
    function moreOfTheSameProduct(id) {
        setSelectedProducts(prev => [...prev, id]);
    }
    function lessOfTheSameProduct(id) {
    ...
    return (
        <Layout>
                ...
                <div className="flex mb-5" key={productInfo._id}> 
                    ...
                    <div className="pl-4">
                        ...
                        <div className="flex">
                            ...
                            <div>
                                <button onClick={() => lessOfTheSameProduct(productInfo._id)} className="border border-emerald-500 px-2 rounded-lg text-emerald-500">-</button>
                                <span className="px-2">{selectedProducts.filter(id => id === productInfo._id).length}</span>
                                <button onClick={() => moreOfTheSameProduct(productInfo._id)} className="bg-emerald-500 px-2 rounded-lg text-white">+</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <form action="/api/checkout" method="POST">
                ...
                <div className="mt-4">
                    ...
                    <div className="flex my-3">
                        ...
                        <h3 className="font-bold">${shippingPrice}</h3>
                    </div>
                    <div className="flex my-3 border-t pt-3 border-dashed border-gray-500">
            ...
            <input type="hidden" name="products" value={selectedProducts.join(',')}/>
                <button className="bg-emerald-500 px-5 py-2 rounded-xl font-bold text-white w-full my-4 shadow-emerald-300 shadow-lg">Place Order</button>
            </form>
        </Layout>
    );
}